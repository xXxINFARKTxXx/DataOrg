...
* проектируется и описывается посредством формализма, который
называется моделью данных
* используется через программный продукт, называемый
...
#### Базы данных и СУБП
###### Система управления базой данных (СУБП)
...
* предоставление распределенной организации БП
* обеспечение назначения схемы базы данных

#### Схема базы данных
###### Программа, использующая услуги СУБП
* знает только схему БП, как логическую категорию

![](lecture1_imgs/image6.png)

![](lecture1_imgs/image7.png)

#### Реляционная база данных
![](lecture1_imgs/image8.png)
...
* формально: S = {(e1,…, em) | P(e1,…, em)}
* ei (i ∈ {1,..., m})
* один реальный сущность или предварительно установленная связь

## Признак, домен и данные
#### Признак
* P(ei), P(e1,…, em):
  * P(ei) означает предикат (свойство) класса сущностей/связей.
  * Выражает характеристики класса E, то есть класса S.
* Признак:
  * Это свойство класса реальных сущностей или связей.
  * Возникает из семантики предиката P(ei).
...
Тип сущности
* формально представляет собой набор явлений: SP(N) = {pi | P(pi)}

Каждое явление pi ∈ SP(N) представляет собой ровно одну реальную сущность ei ∈ E

Задан тип сущности
* с наименованием N и набором характеристик Q = {A1,..., An}
...
* Для каждого Ai ∈ Q справедливо, что ai ∈ dom(Ai)

Если вводится порядок в набор атрибутов типа сущности (A1,..., An),
* тогда появление p(N) рассматривается как n-кортеж (кортеж): (a1,..., an)
...
Формально
* набор атрибутов типа сущности N
...
Тип связи (ТС)
* соединяет две или более ТС или предварительно определенные ТС
* моделирует отношения между проявлениями связанных ТС или ТС, то есть между реальными сущностями или связями
...
## Логическая структура функций
Логическая структура атрибутов (LSO)
...
Возможные подходы к организации LSO (STE, RTE)
...
Уровень детализации визуального представления LSO
...
![](lecture1_imgs/image13.png)

![](lecture1_imgs/image14.png)
